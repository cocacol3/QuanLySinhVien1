...
const addEmployee = function () {
  ...
  isValid &= //&= nghĩa là true hoặc false
  ...
  //nếu có bất cứ chỗ nào false =>false

  if (isValid) {
    // if (lastNameValidation && firstNameValidation) {
    ...
    promise.then(function (res) {
      //Xử lý thành công gọi lại hàm getData lấy dữ liệu mới nhất về
      getData();
...
const updateUser = function () {
  ...
  var promise = empService.update(updateEmpl.id, updateEmpl);
};
...
const getData = function () {
  //dùng axios để call trên api của backend lấy ds nhân viên có sẵn
  ...
  const getData = function () {
    ...
    //   }
    /**
     * 1. Viết hàm map: [EMP1, EMP2] => [new Employee(EMP1), new Employee(EMP2)]
     */
...
//rejector
const rejector = function (err) {
  console.log(err);
}

fetchEmplPromise.then(resolver).catch(rejector);